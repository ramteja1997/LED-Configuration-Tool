...
class FontFileEntry:
    def __init__(self, master, index, remove_callback=None, show_remove=False):
        self.frame = tk.Frame(master, relief=tk.GROOVE, bd=2, padx=15, pady=15)
        self.frame.pack(fill="x", padx=15, pady=15)
        row1 = tk.Frame(self.frame)
        row1.pack(fill="x", pady=(0, 3))
        tk.Label(row1, text=f"TTF file {index + 1}:", width=18).pack(side="left")
        self.path_var = tk.StringVar()
        self.path_entry = tk.Entry(row1, textvariable=self.path_var, width=80)
        ...
        self.range_text = tk.Text(row2, height=3, width=110)
        ...
        link.bind(
            "<Button-1>",
            lambda e: webbrowser.open_new("https://jrgraphix.net/research/unicode.php"),
        )
...
def generate_centum_font_c_multi(font_inputs, out_font_name, size, bpp):
    ...
    for font_path, codepoints in font_inputs:
        ...
        if not supported:
            rng_str = (
                f"{codepoints[0]:#x}"
                if len(codepoints) == 1
                else f"{codepoints[0]:#x}-{codepoints[-1]:#x}"
            )
            raise Exception(
                f"Font \"{os.path.basename(font_path)}\" doesn't have any characters included in range {rng_str}"
            )
    ...
    with open(output_filename, "w", encoding="utf-8") as f:
        ...
        macro_name = base_name.upper().replace(" ", "_").replace("-", "_") + "_CENTUM"
        f.write("#ifdef CENTUM_CONFIG_INCLUDE_SIMPLE\n#include \"centum_config.h\"\n#else\n#include \"centum_config/centum_config.h\"\n#endif\n\n")
        ...
        f.write("static const centum_font_glyph_dsc_t glyph_dsc[] = {\n")
        offset = 0
        for desc in glyph_descs:
            f.write(
                f"    {{ .bitmap_index = {offset}, .adv_w = {desc[3]}, .box_w = {desc[1]},"
        ...
        f.write("static const centum_font_cmap_t cmaps[] = {\n")
        f.write("    {\n")
        f.write(
            f"        .range_start = 0x{unicode_list[0]:04X}, .range_length = {len(unicode_list)},"
            f" .glyph_id_start = 1,\n"
        )
        f.write(
            f"        .unicode_list = unicode_list_0, .glyph_id_ofs_list = NULL,"
            f" .list_length = {len(unicode_list)}, .type = CENTUM_FONT_CMAP_FORMAT0_TINY\n"
        )
        f.write("    }\n};\n\n")

        f.write("static centum_font_txt_desc_t font_dsc = {\n")
        f.write("    .glyph_bitmap = glyph_bitmap,\n")
        f.write("    .glyph_dsc = glyph_dsc,\n")
        f.write("    .cmaps = cmaps,\n")
        f.write("    .kern_dsc = NULL, .kern_scale = 0,\n")
        f.write(
            f"    .cmap_num = 1, .bpp = {bpp}, .kern_classes = 0, .bitmap_format = 0\n"
        )
        f.write("};\n\n")

        safe_name = base_name.replace(" ", "_").replace("-", "_").lower()
        f.write(f"const centum_font_t {safe_name} = {{\n")
        f.write("    .get_glyph_dsc = centum_font_get_glyph_dsc_txt,\n")
        f.write("    .get_glyph_bitmap = centum_font_get_bitmap_txt,\n")
...
def check_block_ranges(block):
    ...
    for (start, end) in ranges:
        ...
        if unsupported_chars:
            ...
            msg = f"Range 0x{start:04X}-0x{end:04X} unsupported, missing characters: {format_range(unsupported_chars)}"
...
class CentumConfigurationToolApp:
    def __init__(self, root):
        ...
        inner_frame = tk.Frame(main_canvas, background="#f0f0f0")
        frame_window = main_canvas.create_window((0, 0), window=inner_frame, anchor="nw")
        inner_frame.bind(
            "<Configure>", lambda event: main_canvas.configure(scrollregion=main_canvas.bbox("all"))
        )
        main_canvas.bind("<Configure>", lambda event: main_canvas.itemconfig(frame_window, width=event.width))

        def on_mousewheel(event):
            main_canvas.yview_scroll(-1 * (event.delta // 120), "units")

        def on_shiftmousewheel(event):
            main_canvas.xview_scroll(-1 * (event.delta // 120), "units")

        main_canvas.bind_all("<MouseWheel>", on_mousewheel)
        main_canvas.bind_all("<Shift-MouseWheel>", on_shiftmousewheel)
        ...
        main_canvas.bind("<ButtonPress-2>", drag_start)
        main_canvas.bind("<B2-Motion>", drag_motion)

        title_label = tk.Label(
            inner_frame,
            text="Centum Configuration Tool",
            font=("Arial", 24, "bold"),
            anchor="center",
            justify="center",
            bg="#f0f0f0",
        ...
        desc_text = (
            "Convert TTF and WOFF fonts to C array for Centum devices.\n\n"
            "With this configuration tool, you can create a C array from any TTF or WOFF. "
            "Select Unicode character ranges and specify the bpp (bit-per-pixel) as needed."
        )
        desc_label = tk.Label(
            inner_frame, text=desc_text, font=("Arial", 12), wraplength=1100, justify="center", bg="#f0f0f0"
        )
        desc_label.pack(pady=(0, 20))

        settings_fr = tk.Frame(inner_frame, pady=5, padx=5, bg="#f0f0f0")
        settings_fr.pack(pady=(0, 16))

        tk.Label(settings_fr, text="Name:", width=12, bg="#f0f0f0").pack(side="left")
        self.font_name_var = tk.StringVar()
        tk.Entry(settings_fr, textvariable=self.font_name_var, width=40).pack(side="left", padx=9)

        tk.Label(settings_fr, text="Font Size (px):", width=18, bg="#f0f0f0").pack(side="left", padx=(19, 0))
        self.font_size_var = tk.IntVar(value=16)
        tk.Spinbox(settings_fr, from_=8, to=72, textvariable=self.font_size_var, width=8).pack(side="left", padx=6)

        tk.Label(settings_fr, text="Bpp:", width=8, bg="#f0f0f0").pack(side="left", padx=(18, 0))
        self.bpp_var = tk.StringVar(value="1")
        ttk.Combobox(settings_fr, textvariable=self.bpp_var, values=["1", "2", "3", "4", "8"], width=5, state="readonly").pack(
            side="left", padx=7
        )

        self.font_block_list = tk.Frame(inner_frame, bg="#f0f0f0")
        self.font_block_list.pack(fill="both", expand=True, padx=5, pady=10)

        self.file_blocks = []
        self.add_font_block(first=True)
        self.buttons_frame = tk.Frame(inner_frame, bg="#f0f0f0")
        self.buttons_frame.pack(pady=(18, 0))

        self.add_font_btn = tk.Button(self.buttons_frame, text="+ Include another font", command=self.add_font_block)
        self.add_font_btn.pack(side="top", pady=10, ipadx=16)

        self.submit_btn = tk.Button(inner_frame, text="Submit", command=self.submit_all, bg="black", fg="white")
        self.submit_btn.pack(pady=(22, 22), ipadx=38)

    def add_font_block(self, first=False):
        show_remove = not first
        idx = len(self.file_blocks)
        block = FontFileEntry(
            self.font_block_list, idx, remove_callback=self.remove_font_block if show_remove else None, show_remove=show_remove
        )
        block.frame.pack(fill="x", padx=7, pady=10)
    ...
    def submit_all(self):
        ...
        if not font_name:
            messagebox.showerror("Input Error", "Please enter a name (top settings).")
            return
        ...
        try:
            out_file = generate_centum_font_c_multi(font_inputs, font_name, int(font_size), int(bpp))
            for block in self.file_blocks:
...
if __name__ == "__main__":
    root = tk.Tk()
    app = CentumConfigurationToolApp(root)
    root.mainloop()
