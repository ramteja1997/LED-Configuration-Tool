    ...
    try:
        ...
        for (start, end) in ranges:
            ...
            if len(supported_chars) == 0:
                ...
                messages.append((f"Range 0x{start:04X}-0x{end:04X} supported with at least one character.", True))
...
class LVGLFontConverterApp:
    def __init__(self, root):
        ...
        tk.Spinbox(settings_fr, from_=8, to=72, textvariable=self.font_size_var, width=5, font=("Segoe UI", 10)).pack(
            side="left", padx=3)

        tk.Label(settings_fr, text="Bpp:", width=6, font=("Segoe UI", 10)).pack(side="left", padx=(15, 0))
        self.bpp_var = tk.StringVar(value='1')
        ttk.Combobox(settings_fr, textvariable=self.bpp_var, values=['1', '2', '3', '4', '8'], width=3, state='readonly',
    ...
    def submit_all(self):
        ...
        # Check all font blocks; each must have at least one supported char in each range
        all_supported = True
        ...
        if not all_supported:
            messagebox.showerror("Font Range Support", "Not all fonts fully support their specified ranges with at least one supported character. Fix errors and try again.")
            return
