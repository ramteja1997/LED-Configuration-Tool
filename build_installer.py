...
# Check for and import psutil and requests, install if missing
# Handling tkinter separately because it often ships with Python and is not always installable via pip
try:
    ...
    for mod_name, pkg_name in required_modules.items():
        if mod_name == "tkinter":
            try:
                ...
                print("Error: Tkinter module is not installed. Please install the appropriate package manually.")
                sys.exit(1)
        else:
            try:
                importlib.import_module(mod_name)
...
required_modules = {
    "psutil": "psutil",
    "requests": "requests",
    "tkinter": "tkinter"  # checked as special case
}
